...
PrettyCalendar.timeToHours = function (formatted) {
  ...
  timeHours =
    Number(timeHours) +
    Number(twoPieces[1].replace("am", "").replace("pm", "")) / 60;
  return timeHours;
};
  ...
  for (var j = 0; j < 7; j++) {
    ...
    if (eventsToday[j].length != 0) {
      eventsToday[j] = eventsToday[j].sort(function (a, b) {
        return (
          PrettyCalendar.timeToHours(a[0]) - PrettyCalendar.timeToHours(b[0])
        );
      });
    }
    for (var i = 0; i < eventsToday[j].length; i++) {
      ...
      if (
        ($("#calendar").height() * lastPercentTemp) / 100 +
          $("#event" + (counterTemp - 1)).innerHeight() +
          PrettyCalendar.EVENT_PADDING >
        ($("#calendar").height() * percentTemp) / 100
      ) {
        numToCompress++;
      } else {
        numToCompress = 1;
      }
      var formatWidth = 100 / 1;
      lastTime = timeHours;
      ...
      if (eventsToday[j][i].length > 3) {
        heightSet =
          "height:" +
          (PrettyCalendar.hoursToPercent(
            PrettyCalendar.timeToHours(eventsToday[j][i][3])
          ) -
            percentTemp) +
          "%;";
      }
      ...
      if (formatWidth != 100) {
        for (var x = 0; x < numToCompress - 1; x++) {
          $("#event" + (counterTemp - (x + 1))).css("width", 100 + "%");
          $("#event" + (counterTemp - (x + 1))).css(
            "left",
            100 - formatWidth * (x + 2) + "%"
          );
          $("#event" + (counterTemp - (x + 1))).attr(
            "title",
            $("#event" + (counterTemp - (x + 1))).text()
          );
        }
...
PrettyCalendar.addNavigation = function () {
  var leftNavBtn =
    '<div class="directionNav" onclick="leftNav()"><span class="directionLabel">&lt;</span></div>';
  var rightNavBtn =
    '<div class="directionNav" onclick="rightNav()" style="left:95%;"><span class="directionLabel">&gt;</span></div>';
